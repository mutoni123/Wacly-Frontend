...
export default function ApplyLeave() {
  ...
  useEffect(() => {
      ...
      const fetchLeaveData = async () => {
          try {
              ...
              toast({
                  ...
                  title: "Success",
                  description: "Leave data loaded successfully"
              });
          } catch (error) {
      ...
      fetchLeaveData();
  }, [isAuthenticated, router, toast, user?.id]);
      // Helper Functions
      const calculateDays = () => {
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            try {
                const statsResponse = await fetch('http://localhost:5000/api/leave-requests/stats', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    }
                ...
                if (statsResponse.ok) {
                    ...
                    if (statsData.success) {
                        const initialBalances = leaveTypes.reduce<{ [key: number]: number }>((acc, type) => {
                            const stat = statsData.data.find(s => s.leave_type_id === type.id);
                            acc[type.id] = type.days_allowed - (stat?.daysUsed || 0);
                            return acc;
                        }, {});
                ...
                toast({
                    variant: "destructive",
                    title: "Warning",
                    description: "Leave request submitted but failed to refresh balances"
                });
            }
        } catch (error) {
